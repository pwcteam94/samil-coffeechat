...
const { TESTIMONIALS, PEER_TOPICS } = require('./lib/seed-data');
...
// ----- 동료(동기) 미팅 신청 -----
function peerView(r) {
  return { id:r.id, status:r.status, createdAt:r.created_at, meetAt:r.meet_at, topic:r.topic,
    from:{ id:r.from_user_id, name:r.from_name, initial:r.from_initial, dept:r.from_dept,
      cohort:r.from_cohort, c1:r.from_c1, c2:r.from_c2 } };
}
// 신규/빈 사용자에게 데모 동료 신청 2~3건을 채워 넣음 (idempotent: 이미 있으면 skip)
function seedIncoming(meId) {
  const has = db.prepare('SELECT COUNT(*) n FROM peer_requests WHERE to_user_id=?').get(meId).n;
  if (has > 0) return;
  const peers = db.prepare("SELECT * FROM users WHERE role='joiner' AND id!=? ORDER BY RANDOM() LIMIT 3").all(meId);
  const topics = [...PEER_TOPICS].sort(() => Math.random() - 0.5);  // 셔플 → 중복 없이 배정
  peers.forEach((pr, i) => {
    db.prepare(`INSERT INTO peer_requests
      (id,to_user_id,from_user_id,from_name,from_initial,from_dept,from_cohort,from_c1,from_c2,topic,message,status,created_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      uid('p_'), meId, pr.id, pr.name, (pr.name||'·')[0], pr.dept, pr.cohort, pr.c1, pr.c2,
      topics[i % topics.length], '', 'pending', now() - (i + 1) * 5400e3);
  });
}
// 네트워크 시각화용 연결 목록: 수락된 선배 + 수락된 동료
function connectionsFor(meId) {
  const seniors = db.prepare(`SELECT r.meet_at, s.id,s.name,s.initial,s.dept,s.c1,s.c2
    FROM requests r JOIN seniors s ON s.id=r.senior_id
    WHERE r.user_id=? AND r.status='accepted'`).all(meId)
    .map(s => ({ kind:'senior', id:s.id, name:s.name, initial:s.initial,
      dept:(s.dept||'').split(' ')[0], c1:s.c1, c2:s.c2, meetAt:s.meet_at }));
  const peers = db.prepare(`SELECT * FROM peer_requests WHERE to_user_id=? AND status='accepted'`).all(meId)
    .map(r => ({ kind:'peer', id:r.from_user_id, name:r.from_name, initial:r.from_initial,
      dept:(r.from_dept||''), c1:r.from_c1, c2:r.from_c2, meetAt:r.meet_at }));
  return [...seniors, ...peers];
}
function pendingForNet(meId) {
  // 보낸 신청 중 대기/거절 (시각화 보조)
  return db.prepare(`SELECT r.status, s.id,s.name,s.initial,s.dept,s.c1,s.c2
    FROM requests r JOIN seniors s ON s.id=r.senior_id
    WHERE r.user_id=? AND r.status!='accepted'`).all(meId)
    .map(s => ({ kind:'senior', status:s.status, id:s.id, name:s.name, initial:s.initial,
      dept:(s.dept||'').split(' ')[0], c1:s.c1, c2:s.c2 }));
}
...
async function api(req, res, url) {
  ...
  if (p === '/api/bootstrap' && m === 'GET') {
    if (me.role === 'joiner') seedIncoming(me.id);
    ...
    const incoming = db.prepare("SELECT * FROM peer_requests WHERE to_user_id=? AND status='pending' ORDER BY created_at DESC").all(me.id).map(peerView);
    return send(res, 200, {
      me, seniors: seniors.map(s => ({ ...s, requested: myReqIds.has(s.id) })),
      requests: myRequests, notifications: notifs, unread,
      incoming, connections: connectionsFor(me.id), pendingNet: pendingForNet(me.id),
  ...
  // --- 동료 신청 수락/거절 ---
  if (p === '/api/peer-requests/respond' && m === 'POST') {
    const r = db.prepare('SELECT * FROM peer_requests WHERE id=? AND to_user_id=?').get(body.id, me.id);
    if (!r) return send(res, 404, { error: 'not found' });
    if (r.status !== 'pending') return send(res, 409, { error: '이미 처리된 신청' });
    const accept = body.decision === 'accept';
    const meet = accept ? genMeet('강남오피스') : null;
    db.prepare('UPDATE peer_requests SET status=?, resolved_at=?, meet_at=? WHERE id=?')
      .run(accept ? 'accepted' : 'declined', now(), meet, r.id);
    if (accept) {
      const courage = me.courage + 20;
      db.prepare('UPDATE users SET courage=?, level=? WHERE id=?').run(courage, levelFor(courage), me.id);
      notify(me.id, { type: 'peer', title: `🤝 ${r.from_name} 동기와 연결됐어요!`,
        body: `${meet} 미팅 예정. 동료 연결 +20점이 적립됐어요.` });
    }
    const meNow = db.prepare('SELECT * FROM users WHERE id=?').get(me.id);
    return send(res, 200, {
      ok: true, me: meNow, nextLevel: nextLevelPts(meNow.level),
      incoming: db.prepare("SELECT * FROM peer_requests WHERE to_user_id=? AND status='pending' ORDER BY created_at DESC").all(me.id).map(peerView),
      connections: connectionsFor(me.id),
    });
  }
